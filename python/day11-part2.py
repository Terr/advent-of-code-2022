...
class Monkey:
    def __init__(
        ...
        test_value: int,
        to_monkey_true: MonkeyId,
        to_monkey_false: MonkeyId,
    ):
        ...
        self.items: list[WorryLevel] = []
        self.inspection_counter = 0

    def distribute_items(self):
        while len(self.items) > 0:
            to_monkey_id = self.inspect_item(0)
            worry_level = self.throw_item(0)

            yield worry_level, to_monkey_id

    def inspect_item(self, item_index: int) -> MonkeyId:
        self.inspection_counter += 1

        old = self.items[item_index]
        new = eval(self.inspect_operation)
    ...
    def throw_item(self, item_index: int) -> WorryLevel:
        return self.items.pop(item_index)

    def catch_item(self, item_worry_level: WorryLevel):
        self.items.append(item_worry_level)

    def normalize_worry_levels(self, scale: int):
        self.items = list(map(lambda wl: wl % scale, self.items))


def parse_puzzle_input(lines: list[str]) -> list[Monkey]:
    monkeys = []

    for line_num in range(0, len(lines), 7):
        # Gather the specifics about the behaviour of this monkey, skipping the
        # first line (the "Monkey (ID):" line) and the newline at the end of a
        # text block.
        monkey_data = list(
            map(lambda l: l.strip().split(":")[1], lines[line_num + 1 : line_num + 6])
        )

        inspect_operation = monkey_data[1].split("=")[1].strip()
        # Hey, you never know!
        assert "import" not in inspect_operation

        test_value = int(monkey_data[2].split()[2])
        to_monkey_true = int(monkey_data[3].split()[3])
        to_monkey_false = int(monkey_data[4].split()[3])

        monkey = Monkey(inspect_operation, test_value, to_monkey_true, to_monkey_false)
        # (Worry level of) items the money starts out with
        for worry_level in monkey_data[0].split(","):
            monkey.catch_item(WorryLevel(worry_level))
...
# puzzle_input = open("../puzzle-input/day11-example-input.txt").readlines()
puzzle_input = open("../puzzle-input/day11-input.txt").readlines()
monkeys = parse_puzzle_input(puzzle_input)

# To keep the worry levels from spiraling out of control we can adjust them by
# taking their modulo of the Least Common Multiple (LCM) value of all 'test
# values' to keep the number small, without affecting the divisibility of the
# worry levels by the test values.
#
# Because all the test values are primes, their Least Common Multiple (LCM) is
# their product
test_value_least_common_multiple = math.prod([m.test_value for m in monkeys])

for r in range(NUM_ROUNDS):
    for monkey in monkeys:
        monkey.normalize_worry_levels(test_value_least_common_multiple)

        for item_worry_level, to_monkey_id in monkey.distribute_items():
            monkeys[to_monkey_id].catch_item(item_worry_level)

top_inspection_counts = sorted([m.inspection_counter for m in monkeys], reverse=True)
