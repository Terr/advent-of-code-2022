...
class Monkey:
    def __init__(
        ...
        self.resolved_value: Optional[int] = resolved_value
        self.dependencies: Optional[tuple[str, str]] = dependencies
...
def parse_monkeys(lines: list[str]) -> dict[str, Monkey]:
    ...
    for line in lines:
        ...
        if len(parts) == 2:
            ...
            monkey = Monkey(dependencies=(parts[1], parts[3]), operation=parts[2])
...
def resolve_value(monkey: Monkey, monkeys: dict[str, Monkey]) -> int:
    if monkey.resolved_value is not None:
        return monkey.resolved_value

    dependency_1 = monkeys[monkey.dependencies[0]]
    dependency_2 = monkeys[monkey.dependencies[1]]

    if dependency_1.resolved_value is None:
        resolve_value(dependency_1, monkeys)

    if dependency_2.resolved_value is None:
        resolve_value(dependency_2, monkeys)

    value_1 = dependency_1.resolved_value
    value_2 = dependency_2.resolved_value
...
print(resolve_value(monkeys["root"], monkeys))
