...
def print_elf_positions(elves: set[Coordinates]):
    # Sort the elves in row-order
    sorted_elves = sorted(elves, key=lambda elf: (elf.y, elf.x))
    lowest_x = min([elf.x for elf in elves])
...
def elf_area(elves: set[Coordinates]) -> tuple[Coordinates, Coordinates]:
    """Returns the coordinates of a rectangle that covers all elves'
    positions.
    """

    top = sorted(elves, key=lambda elf: elf.y)[0].y
    left = sorted(elves, key=lambda elf: elf.x)[0].x
    top_left = Coordinates(left, top)

    bottom = sorted(elves, reverse=True, key=lambda elf: elf.y)[0].y
    right = sorted(elves, reverse=True, key=lambda elf: elf.x)[0].x
    bottom_right = Coordinates(right, bottom)
...
area = elf_area(elves)
num_tiles = (1 + area[1].x - area[0].x) * (1 + area[1].y - area[0].y)
print(num_tiles - len(elves))
