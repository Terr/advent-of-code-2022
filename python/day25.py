...
class Snafu:
    ...
    @classmethod
    def to_decimal(cls, snafu: str) -> int:
        decimal = 0

        # Read the SNAFU number from right to left
        for position, symbol in enumerate(reversed(snafu)):
            match symbol:
                case ("0" | "1" | "2"):
                    value = int(symbol)
                ...
                case _:
                    raise Exception("Encountered unexpected symbol: %s" % symbol)

            decimal += value * (cls.BASE**position)
    ...
    def __add__(self, other) -> "Snafu":
        cls = self.__class__
        if type(other) == cls:
            return cls.from_decimal(self.as_decimal() + other.as_decimal())
        elif type(other) == str:
...
def run_tests():
    assert Snafu.to_decimal("2=-01") == 976
    assert Snafu.to_decimal("1121-1110-1=0") == 314159265

    assert Snafu.from_decimal(3) == "1="
    assert Snafu.from_decimal(4) == "1-"
    assert Snafu.from_decimal(5) == "10"
    assert Snafu.from_decimal(6) == "11"
    assert Snafu.from_decimal(14) == "1=-"
    assert Snafu.from_decimal(15) == "1=0"
    assert Snafu.from_decimal(20) == "1-0"
    assert Snafu.from_decimal(2022) == "1=11-2"
    assert Snafu.from_decimal(12345) == "1-0---0"
    assert Snafu.from_decimal(314159265) == "1121-1110-1=0"


run_tests()

# fuel_requirements = open("../puzzle-input/day25-example-input.txt").readlines()
fuel_requirements = open("../puzzle-input/day25-input.txt").readlines()

sum_snafu = Snafu("0")
for requirement in fuel_requirements:
    sum_snafu += requirement.strip()

print(sum_snafu.value)
