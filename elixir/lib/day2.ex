defmodule Day2 do
  ...
  *  The third round is a draw with both players choosing Scissors, giving you a
  ...
  # The value on the right wins from the key on the left
  @winning_moves %{
    @rock => @paper,
    @paper => @scissors,
    @scissors => @rock
  ...
  def solve(input_path) do
    input_path
    |> parse()
    |> List.foldl(0, fn {enemy_move, your_move}, score ->
      case round_outcome(enemy_move, your_move) do
        :right_wins -> score + your_move + 6
        :left_wins -> score + your_move
        :tie -> score + your_move + 3
      end
    end)
  end

  defp parse(input_path) do
    File.read!(input_path)
    |> String.split("\n", trim: true)
    |> Enum.map(&String.split/1)
    |> Enum.map(fn [left, right] -> {@symbol_map[left], @symbol_map[right]} end)
  end

  defp round_outcome(left, right) do
    cond do
      left == right -> :tie
      @winning_moves[left] == right -> :right_wins
      true -> :left_wins
    end
  end
end
