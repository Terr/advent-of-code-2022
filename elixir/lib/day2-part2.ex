defmodule Day2Part2 do
  ...
  *  The third round is a draw with both players choosing Scissors, giving you a
  ...
  # The value on the right *loses* from the key on the left
  @losing_moves %{
    @rock => @scissors,
    @paper => @rock,
    @scissors => @paper
  ...
  def solve(input_path) do
    input_path
    |> parse()
    |> List.foldl(0, fn {enemy_move, desired_outcome}, score ->
      case desired_outcome do
        :should_lose -> score + @losing_moves[enemy_move]
        :should_win -> score + @winning_moves[enemy_move] + 6
        :should_tie -> score + enemy_move + 3
      end
    end)
  end
